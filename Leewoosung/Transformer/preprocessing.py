...
def prepare_transformer_input(train_df, test_df, target_col):
...
def preprocess_for_transformer(train_path, test_path, output_dir):
    target_col = '임신 성공 여부'
    ...
    X_train, X_val, X_test, y_train, y_val = prepare_transformer_input(train_df, test_df, target_col)
    save_processed_data(output_dir, X_train, X_val, X_test, y_train, y_val)

# 실행 코드
train_path = "/content/drive/MyDrive/opentrain.csv"
test_path = "/content/drive/MyDrive/open/test.csv"
output_dir = "/content/drive/MyDrive/open/Transformer/preprocessed_data"
preprocess_for_transformer(train_path, test_path, output_dir)
