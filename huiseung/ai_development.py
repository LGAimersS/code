        ...
        if isinstance(X, pd.DataFrame):
            ...
            if is_training:
                ...
                if y is not None:
                    ...
                    # 원본 특성에 대해 feature selection 적용
                    all_features = pd.concat([X, X_poly_df[self.selected_features]], axis=1)
                    ...
                    X_selected = pd.concat([X, X_poly_df], axis=1).values
            else:
                # 예측 시 저장된 변환 사용
                ...
                # 선택된 특성만 결합
                all_features = pd.concat([X, X_poly_df], axis=1)
                X_selected = self.feature_selector.transform(all_features)
...
if __name__ == "__main__":
    ...
    # 모델 초기화 및 학습
    ensemble = EnhancedFertilityEnsemble(n_components=150, use_optuna=True)
    print("모델 학습 시작...")
    results = ensemble.train(X, y, batch_size=32, n_splits=5)

    # 테스트 데이터에 대한 예측
    print("\n테스트 데이터 예측 중...")
    confidence_results = ensemble.predict_with_confidence(test_data, n_bootstrap=20)

    # 결과 저장
    sample_submission['probability'] = confidence_results['probabilities']
    sample_submission['prediction'] = confidence_results['predictions']
    sample_submission['std_dev'] = confidence_results['std_dev']
    sample_submission['lower_ci'] = confidence_results['lower_ci']
    sample_submission['upper_ci'] = confidence_results['upper_ci']

    # 불확실성이 높은 샘플 식별 (신뢰구간이 결정 경계 0.5를 포함하는 경우)
    sample_submission['uncertain'] = ((confidence_results['lower_ci'] < 0.5) &
                                    (confidence_results['upper_ci'] > 0.5)).astype(int)
    ...
    print(f"\n불확실한 예측 샘플 비율: {sample_submission['uncertain'].mean():.2%}")