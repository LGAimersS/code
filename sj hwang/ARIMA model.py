...
# ARIMA 모델 학습 및 예측 함수
def train_and_evaluate_arima(train_path, test_path, method):
    try:
        # 1️⃣ 데이터 로드
        print(f"\n📂 데이터 로드 중... ({method})")
        ...
        # 5️⃣ 최적의 ARIMA 파라미터 찾기 (auto_arima 사용)
        print(f"🔍 최적의 ARIMA 파라미터 찾는 중... ({method})")
        optimal_arima = auto_arima(y_train, seasonal=False, trace=True)
        best_order = optimal_arima.order
        print(f"✅ 최적의 ARIMA 파라미터: {best_order}")

        # 6️⃣ ARIMA 모델 훈련
        print(f"🚀 ARIMA 모델 학습 시작... ({method})")
        ...
        # 8️⃣ 평가 지표 출력
        evaluate_model(y_train[-len(y_pred):], y_pred, f"ARIMA_{method}")

        # 9️⃣ 시각화 (실제값 vs 예측값 비교)
        plot_predictions(y_train[-len(y_pred):], y_pred, f"ARIMA_{method}")
        plot_residuals(y_train[-len(y_pred):], y_pred, f"ARIMA_{method}")

        # 🔟 원본 테스트 데이터와 병합하여 모든 ID 포함
        original_test_df = pd.read_csv("./datasets/test.csv")[["ID"]]
        arima_results = pd.DataFrame({"ID": test_df["ID"], "probability": y_pred})

        final_submission = original_test_df.merge(arima_results, on="ID", how="left")
        final_submission["probability"] = final_submission["probability"].fillna(0)

        # 1️⃣1️⃣ 결과 저장
        output_filename = f"./Arima result/ARIMA_{method}_sample.csv"
...
if __name__ == "__main__":
    ...
    for method in methods:
        train_and_evaluate_arima(train_files[method], test_files[method], method)
